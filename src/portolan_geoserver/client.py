...
class EmptyGeoServerClient:
    """Read-only client for offline planning."""

    def list_portolan_resources(self, workspace: str) -> dict[str, dict[str, Any]]:
        """Return no existing managed resources."""
        return {}

    def ensure_workspace(self, workspace: str) -> None:
        """Offline planning cannot mutate GeoServer."""
        raise RuntimeError("offline GeoServer client cannot publish")
...
class GeoServerClient:
    ...
    def publish_geoparquet(
        ...
        """Create a GeoParquet store and FeatureType."""
    ...
    def publish_cog(
        ...
        """Create a COG coverage store and Coverage."""
    ...
    def _collect_feature_type(
        ...
        if not isinstance(feature_type, dict):
            return
        metadata = metadata_entries(feature_type.get("metadata"))
    ...
    def _collect_coverage(
        ...
        if status >= 400 or not isinstance(coverage, dict):
            return
        metadata = metadata_entries(coverage.get("metadata"))
        collection = metadata.get("portolan.collection")
        if _is_managed(metadata.get("portolan.managed")) and isinstance(collection, str):
            managed[collection] = metadata
    ...
    def _coverage_stores(self, workspace: str) -> tuple[list[str], int]:
        rest_service = self._client.rest_service
        response = rest_service.rest_client.get(
            rest_service.rest_endpoints.coveragestores(workspace)
        )
        if response.status_code >= 400:
            return [], response.status_code
        return coverage_store_names(response.json()), response.status_code

    def _put_feature_type_metadata(
        self, workspace: str, datastore: str, name: str, metadata: dict[str, object]
    ) -> None:
        payload = {"featureType": {"name": name, "metadata": metadata_payload(metadata)}}
        rest_service = self._client.rest_service
    ...
    def _put_coverage_metadata(
        ...
        payload = {"coverage": {"name": name, "metadata": metadata_payload(metadata)}}
        rest_service = self._client.rest_service
...
def metadata_payload(metadata: dict[str, object]) -> dict[str, object]:
    """Return GeoServer REST metadata XML/JSON shape."""
    return {
        "entry": [
...
def metadata_entries(metadata: object) -> dict[str, Any]:
    """Return flat metadata entries from GeoServer REST payload variants."""
